...
public class CompteDaoImpl implements CompteDao {
    ...
    private static final String SQL_SELECT_PAR_TYPECOMPTE = "select * from compte where TypeCompte = ?";
    private static final String SQL_INSET = "insert into compte(Courriel,Password,TypeCompte) value(?,?,?)";
    ...
    public List<Compte> findByPassword(String password) {
        ...
        try {
            //Initialise la requête préparée basée sur la connexion 
            // la requête SQL passé en argument pour construire l'objet preparedStatement
            PreparedStatement ps = ConnexionBD.getConnection().prepareStatement(SQL_SELECT_PAR_PASSWORD);
    ...
    public List<Compte> findByTypeCompte(String type) {
        ...
        try {
            //Initialise la requête préparée basée sur la connexion 
            // la requête SQL passé en argument pour construire l'objet preparedStatement
            PreparedStatement ps = ConnexionBD.getConnection().prepareStatement(SQL_SELECT_PAR_TYPECOMPTE);
            ps.setString(1, type);
            //On execute la requête et on récupère les résultats dans la requête 
            // dans ResultSet
            ResultSet result = ps.executeQuery();
